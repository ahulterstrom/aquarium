...
import { Tank, VisitorInterests, GridCell, GridPosition } from '../types/game.types';
...
export class WaypointSystem {
  ...
  constructor(gridSize: { width: number; depth: number }, gridStore: GridStoreInterface) {
  ...
  private generateTankViewingWaypoints(tank: Tank): void {
    ...
    for (let i = 0; i < directions.length; i++) {
      ...
      const viewingPos = tankPos.clone().add(direction.clone().multiplyScalar(viewingDistance));
  ...
  private generateRestWaypoints(tanks: Map<string, Tank>): void {
    const corners = [
      ...
      new THREE.Vector3((this.gridSize.width - 1) * 2 - 0.5, 0.5, (this.gridSize.depth - 1) * 2 - 0.5)
    ];
  ...
  findBestWaypoint(
    ...
    for (const waypoint of Array.from(this.waypoints.values())) {
      ...
      if (waypoint.associatedTankId && visitedTankIds.includes(waypoint.associatedTankId)) {
        continue;
      }
      
      const score = this.calculateWaypointScore(waypoint, currentPosition, interests);
  ...
  private isPositionValid(position: THREE.Vector3): boolean {
    ...
    // Check bounds
    if (gridX < 0 || gridX >= this.gridSize.width || gridZ < 0 || gridZ >= this.gridSize.depth) {
      return false;
    }
  ...
  private isPositionValidForExploration(
    ...
    for (let dx = -checkRadius; dx <= checkRadius; dx++) {
      for (let dz = -checkRadius; dz <= checkRadius; dz++) {
        ...
        if (checkX < 0 || checkX >= this.gridSize.width || checkZ < 0 || checkZ >= this.gridSize.depth) {
          continue;
        }
