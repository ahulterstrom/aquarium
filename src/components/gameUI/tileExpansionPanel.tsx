...
export const TileExpansionPanel = () => {
  ...
  const currentGridSize = 9 + placedExpansionTiles.size; // 3x3 original + expansions
  ...
  if (isInPlacementMode) {
    // Show placement controls overlay when in placement mode
    return (
      <div className="pointer-events-auto fixed bottom-4 left-1/2 z-50 -translate-x-1/2 transform">
        <div className="flex items-center gap-4 rounded-lg border bg-white p-4 shadow-lg">
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="outline" onClick={handleCancelPlacement}>
      ...
      <SheetContent side="bottom" className="mx-auto max-w-2xl">
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-lg bg-blue-50 p-4">
              <div className="mb-2 flex items-center gap-2">
                <Grid3X3 className="h-5 w-5 text-blue-600" />
                <span className="font-medium text-blue-800">Current Size</span>
              </div>
              <div className="text-2xl font-bold text-blue-900">
                {currentGridSize} tiles
              </div>
              <div className="text-sm text-blue-700">
                {placedExpansionTiles.size} expansion tiles placed
              </div>
            </div>
          ...
          <div className="rounded-lg border p-4">
            <div className="mb-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Expand your aquarium by placing tiles adjacent to existing
                  ones
                </p>
              </div>
            </div>
          ...
          <div className="rounded-lg bg-gray-50 p-4 text-sm text-gray-700">
            <h4 className="mb-2 font-medium">Placement Rules:</h4>
            <ul className="space-y-1">
              <li>
                • Tiles must share a side (not diagonal) with existing tiles
              </li>
              <li>• Cannot place tiles next to entrances</li>
              <li>• Click tiles to select/deselect during placement</li>
            </ul>
          </div>
        </div>
      </SheetContent>
