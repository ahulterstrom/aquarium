...
export const getCharactersByGender = (gender: 'male' | 'female' | 'neutral'): CharacterModel[] => {
...
export const getRandomCharacter = (filter?: {
  ...
  if (filter?.tags && filter.tags.length > 0) {
    candidates = candidates.filter(
      model => model.tags?.some(tag => filter.tags!.includes(tag))
    );
  }
...
// Character distribution settings
export const CHARACTER_DISTRIBUTION = {
  // Ensure variety by limiting how many of the same model can appear
  maxSameModelRatio: 0.2, // Max 20% of visitors can use the same model
  
  // Preferred tags based on time of day (future feature)
  timeBasedTags: {
    morning: ['casual', 'worker'],
    afternoon: ['family', 'tourist'],
    evening: ['formal', 'couple'],
    night: ['young', 'party'],
  },
};
